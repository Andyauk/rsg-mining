...
AddEventHandler('rsg-mining:server:giveMiningReward', function()
	...
	if chance <= 50 then
		...
		TriggerClientEvent('QRCore:Notify', src, 'small haul time!', 'primary')
	elseif chance >= 50 and chance <= 80 then -- medium reward
        ...
        TriggerClientEvent("inventory:client:ItemBox", src, QRCore.Shared.Items[item3], "add")
		TriggerClientEvent('QRCore:Notify', src, 'medium haul this time!', 'primary')
	elseif chance > 80 then -- large reward
        ...
        TriggerClientEvent("inventory:client:ItemBox", src, QRCore.Shared.Items[item3], "add")
		TriggerClientEvent('QRCore:Notify', src, 'large haul this time!', 'primary')
	end
end)
...
AddEventHandler('rsg-mining:server:removeitem', function(item)
	...
	if item == 'pickaxe' then
		...
		QRCore.Functions.Notify(src, 'your pickaxe broke!', 'success')
	else
